...
dataset_train, dataset_test, target_train, target_test = train_test_split(dataset, target, test_size=0.2,random_state=1)
...
print("-"*50)
all_categorized_number,all_categorized_index_names  = pd.factorize(target)

# real_target = pd.factorize(target_test)[0]

best_estimator = {'bootstrap': True, 'max_depth': 20, 'max_features': 'sqrt', 'min_samples_leaf': 1, 'min_samples_split': 2, 'n_estimators': 1000}
...
# default_Random_Forest_clf.fit(dataset, all_categorized_number)
...
def resultView(name):
    count = 0
    k = 0
    writer = pd.ExcelWriter(os.getcwd() + "/../data/output_" + name + "_clf_confusion_matrix.xlsx")
    ...
    for i in target_test.index:
        ...
        value_list = [real_value.index.get_values(),  prediction[k], real_value.get_values(), real_value.get_values() == prediction[k]]
    ...
    writer.save()
    DF.to_csv(os.getcwd() + "/../data/output_" + name + "_clf.csv", encoding="utf-8")
    print("count:", count)
...
def resultTestView(name): # RandomForest_D0
    ...
    confusion_matrix.to_excel(writer, "Sheet1")
    writer.save()
    DF.to_csv(os.getcwd() + "/../data/test_output_" + name + "_clf.csv", encoding="utf-8")

# print("-"*80)
# resultView("RandomForest_D0_noDir")
print("-"*80)
# print(resultAllView("RandomForest_D0_noDir"))
print("-"*80)
resultTestView("RandomForest_D0_noDir")
# print("-"*80)
